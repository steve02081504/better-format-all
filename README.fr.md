...
- L'état est stocké dans `.git/better-format-all.json` (configurable via `betterFormatAll.stateFile`). Il associe un chemin relatif au dépôt au SHA du commit auquel ce chemin a été entièrement formaté pour la dernière fois ; la racine du dépôt est la chaîne vide `''`.
...
| ---------------------------------- | ------------------------ | ------------------------------------------------------------- |
| `betterFormatAll.stateFile`        | `better-format-all.json` | Nom du fichier écrit dans le répertoire `.git` du dépôt.      |
| `betterFormatAll.includeUntracked` | `true`                   | Formate aussi les fichiers non suivis et non ignorés par git. |
