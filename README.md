...
2. Choose **Format All Files in Folder**.
...
- The state lives in `.git/format-all.json` (configurable with `formatAll.stateFile`). It maps a repository-relative path to the commit SHA at which that path was last fully formatted; the repository root is the empty string `''`.
